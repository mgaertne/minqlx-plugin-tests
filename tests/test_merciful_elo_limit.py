...
class TestMercifulEloLimit:
    ...
    def test_handle_player_connect_with_no_remaining_application_games(self, merciful_db, game_in_progress):
        ...
        assert_that(
            return_code,
            matches("You used up all your.*application matches. Next application game available at.*"),
        )
    ...
    def test_cmd_merci_grants_another_application_match_by_steam_id(self, merciful_db, chat_channel):
        ...
        # noinspection PyTypeChecker
        return_code = self.plugin.cmd_merci(player, f"!merci {tracked_player.steam_id}".split(), minqlx.CHAT_CHANNEL)
    ...
    def test_cmd_merci_grants_another_application_match_by_id_in_game(self, merciful_db, chat_channel):
        ...
        # noinspection PyTypeChecker
        return_code = self.plugin.cmd_merci(player, f"!merci {tracked_player.id}".split(), minqlx.CHAT_CHANNEL)
...
class TestConnectThread:
    ...
    def test_run_with_wrong_player_in_qlstats_response(self, qlstats_response, mocked_logger):
        when(qlstats_response).json().thenReturn({"players": [{"steamid": str(456), "ca": {"elo": 1234}}]})
    ...
    def test_proper_qlstats_response(self, qlstats_response):
        when(qlstats_response).json().thenReturn({"players": [{"steamid": str(123), "ca": {"elo": 1234}}]})
