...
class TestFragStats:
    ...
    def test_cmd_soulzbalance_returns_best5_worst_5(self, mock_channel, fragstats_db):
        ...
        mock_channel.assert_was_replied(matches(
    ...
    def test_cmd_soulzbalance_for_another_player(self, mock_channel, fragstats_db):
        ...
        when(fragstats_db).zscore(f"minqlx:players:{player.steam_id}:soulz",
                                  fragging_player.steam_id)\
            .thenReturn(3)
        when(fragstats_db).zscore(f"minqlx:players:{player.steam_id}:reaperz",
                                  fragging_player.steam_id)\
            .thenReturn(None)
    ...
    def test_cmd_soulzbalance_for_another_disconnected_player(self, mock_channel, fragstats_db):
        ...
        when(fragstats_db).zscore(f"minqlx:players:{player.steam_id}:soulz", fragging_player.steam_id)\
            .thenReturn(2)
        when(fragstats_db).zscore(f"minqlx:players:{player.steam_id}:reaperz", fragging_player.steam_id)\
            .thenReturn(6)
