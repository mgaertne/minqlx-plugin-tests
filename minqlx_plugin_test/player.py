...
def fake_player(steam_id, name, team="spectator", ping=0):
    ...
    **Make sure to use :func:`mockito.unstub()` after calling this function to avoid side effects spilling into the next test.**
    ...
    :return: a mocked player that might be used to set up the game and interactions can be checked with assertion functions afterwards.
    """
...
def connected_players(plugin, *players):
    ...
    **Make sure to use :func:`mockito.unstub()` after calling this function to avoid side effects spilling into the next test.**
    ...
    :param players: the players that are currently on the server, in all possible teams: "red", "blue", "spectator", and "free"
    """
...
def assert_player_was_put_on(player, matcher, times=1):
    ...
    **Make sure to use :func:`mockito.unstub()` after calling this assertion to avoid side effects spilling into the next test.**
    ...
    :param matcher: matches the team the player should have been put on. This might be  :class:`mockito.matchers` to check that the player was not put on a different team.
    :param times: The amount of times the player should have been put on the matching team, set to 0 for team switch for that player to have happened. (default: 1).
    """
...
def assert_player_was_told(player, matcher, times=1):
    ...
    **Make sure to use :func:`mockito.unstub()` after calling this assertion to avoid side effects spilling into the next test.**
    ...
    :param matcher: matches the text the player should have told. This might be  :class:`mockito.matchers` to check for certain types of messages.
    :param times: The amount of times the player should have been told the matching message, set to 0 for no message told to the player. (default: 1).
    :return:
    """
