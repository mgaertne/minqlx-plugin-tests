...
class showdown(Plugin):
    ...
    def allow_music(self):
        ...
        return self.game.blue_score != self.game.roundlimit - 1 and self.game.red_score != self.game.roundlimit - 1
    ...
    def weapon_showdown(self, preselected_weapon=None):
        ...
        random_weapon = preselected_weapon if preselected_weapon else next(self.random_weapons_iter)
...
class Weapon:
    ...
    def is_identified_by(self, text):
        ...
        return any(aliases.lower().strip() == text.lower().strip() for aliases in self.aliases)
