...
class SubscriberCog(Cog):
    ...
    async def _subscribe_map(self, interaction, mapname):
        ...
        if not db_return_value:
            ...
            immediate_reply_message = (
                f"You have been subscribed to map changes for map `{self.formatted_installed_maps[stripped_mapname]}`."
            )
        ...
        reply_embed.description = (
            f"{immediate_reply_message}\nYou are currently subscribed to map changes for: `{formatted_maps}`"
        )
        await interaction.edit_original_response(embed=reply_embed)
    ...
    async def _subscribe_player(self, interaction, player):
        ...
        reply_embed.description = (
            f"{immediate_reply_message}\nYou are currently subscribed to the following players: `{formatted_players}`"
        )
        await interaction.edit_original_response(embed=reply_embed)
    ...
    async def _subscribe_member(self, interaction, member):
        ...
        reply_embed.description = (
            f"{immediate_reply_message}\nYou are currently subscribed to Quake Live activities of: {formatted_users}"
        )
        await interaction.edit_original_response(embed=reply_embed)
    ...
    async def _unsubscribe_map(self, interaction, mapname):
        ...
        if not db_return_value:
            immediate_reply_message = (
                f"You were not subscribed to map changes for map `{self.format_mapname(stripped_mapname)}`. "
            )
        else:
            immediate_reply_message = (
                f"You have been unsubscribed from map changes for map `{self.format_mapname(stripped_mapname)}`. "
            )
        reply_embed.description = immediate_reply_message
        await interaction.edit_original_response(embed=reply_embed)
    ...
    async def _unsubscribe_member(self, interaction, member):
        ...
        if len(subscribed_users) == 0:
            reply_embed.description = (
                f"{immediate_reply_message}\nYou are no longer subscribed to Quake Live activities of anyone."
            )
            await interaction.edit_original_response(embed=reply_embed)
            return
        ...
        formatted_users = ", ".join([user.mention for user in subscribed_users])
        reply_embed.description = (
            f"{immediate_reply_message}\nYou are still subscribed to Quake Live activities of {formatted_users}"
        )
        await interaction.edit_original_response(embed=reply_embed)
    ...
    async def notify_player_connected(self, player):
        ...
        for key in self.db.keys(DISCORD_PLAYER_SUBSCRIPTION_KEY.format("*")):
            if self.db.sismember(key, player.steam_id):
                ...
                notifications.append(
                    subscribed_discord_user.send(
                        content=f"`{player.clean_name}`, one of your followed players, just connected to the server!"
                    )
                )
    ...
    async def check_subscriptions(self):
        ...
        game = None
        with contextlib.suppress(NonexistentGameError):
            game = minqlx.Game()
