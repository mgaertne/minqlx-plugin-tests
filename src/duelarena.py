...
class duelarena(Plugin):
    ...
    def handle_player_loaded(self, player):
        ...
        if len(self.duelarena_game.playerset) + 1 == MIN_ACTIVE_PLAYERS:
            player.tell(
                f"{player.name}, join to activate DuelArena! Round winner stays in, loser rotates with spectator."
            )
...
class DuelArenaGame:
    ...
    def check_for_activation_or_abortion(self):
        ...
        if (
            self.is_activated()
            and (len(self.playerset) > MIN_ACTIVE_PLAYERS or len(self.playerset) > MAX_ACTIVE_PLAYERS)
